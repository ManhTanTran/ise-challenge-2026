...
def has_word(normalized: str, term: str) -> bool:
    """Whole-word match on already-normalized (normalize_for_match'd) text.

    Prevents short keyword cues from matching inside longer words - e.g. the
    Vietnamese "hinh" (image) must not fire on "chinh" (main). Allows an
    optional trailing "s" so English plurals ("slots", "scholarships") still
    match the singular term list.
    """

    return re.search(rf"(?<![a-z0-9]){re.escape(term)}s?(?![a-z0-9])", normalized) is not None
