"""Buoc 0a: manifest and chunk construction over the data lake."""
...
def load_or_build_manifest(
    ...
    *,
    ...
    """Load an existing manifest or scan the data lake to build one."""
...
def load_or_build_chunks(
    ...
    *,
    ...
    """Split extracted text plus metadata into retrieval chunks."""
    ...
    for item in manifest:
        text = normalize_spaces(f"{_metadata_text(item)}\n{load_index_text(item, limit=120000)}")
        parts = chunk_text(text, max_chars=max_chars, overlap=overlap) or [_metadata_text(item)]
