...
def load_questions(question_path: str | Path) -> pd.DataFrame:
    ...
    if path.suffix.lower() in {".xlsx", ".xls"}:
        # Benchmark workbooks may contain a title and a note above the actual
        # table. Locate the header row instead of assuming it is always row 1.
        raw = pd.read_excel(path, header=None)
        header_row = next(
            (
                index
                for index, row in raw.iterrows()
                if any(_normalize_column_name(value) == "question" for value in row)
            ),
            0,
        )
        dataframe = pd.read_excel(path, header=header_row)
    ...
    data_sources_column = column_map.get("data_sources") or column_map.get("evidences")
